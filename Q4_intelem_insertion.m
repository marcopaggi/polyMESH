...
isotropic_polycrystal=false;    % this parameter permits to generate meshes
...
Xe=zeros(nb_elements,5); 
Ye=zeros(nb_elements,5); 
Ze=zeros(nb_elements,5); 
edge=zeros(nb_elements,5); 
z=zeros(nb_elements,4); 
...
for e=1:nb_elements
    Be(e,1)=(Xe(e,1)+Xe(e,2)+Xe(e,3)+Xe(e,4))/4;
    Be(e,2)=(Ye(e,1)+Ye(e,2)+Ye(e,3)+Ye(e,4))/4;
    Be(e,3)=(Ze(e,1)+Ze(e,2)+Ze(e,3)+Ze(e,4))/4;
end
...
% 4 vertices of each finite element. New coordinates are the old ones
% multiplied by the scale factor.
% T=(e g x1 y1 x2 y2 x3 y3 x4 y4 x1c y1c x2c y2c x3c y3c x4c y4c)
T=zeros(nb_elements,18);
g=1;
grain_id=gmsh_els(1,5);
for e=1:nb_elements
    if (gmsh_els(e,5)==grain_id)
    ...
    c4c=f_scale.*(c4-cb)+cb;
    
    T(e,:)=[e grain_id c1 c2 c3 c4 c1c c2c c3c c4c];
end

clear c1 c2 c3 c4 c1c c2c c3c c4c cb
clear Ae Ag Sum_AeXe Sum_AeYe Xe Ye Ze

% Connectivity matrix of the finite elements containing the shrunk 
% coordinates
Xsh=T(1:nb_elements,[11 13 15 17 11]);
Ysh=T(1:nb_elements,[12 14 16 18 12]);
...
hold on
for e=1:nb_elements
    plot(Xsh(e,1:4+1),Ysh(e,1:4+1),'-b', 'LineWidth',1)
    hold on
end
...
sum=1;
inew=0;
connect_el=zeros(nb_elements,5);
for e=1:nb_elements
    connect_el(e,1)=e;
    for n=1:4
...
for i=1:nb_seg
    ...
    scalar(i)=-compy(i)*compxg(i)+compx(i)*compyg(i);                   % clockwise rotation of 90 deg
    if (scalar(i)>0)
...
feapelems=[connect_el(:,1),zerov1',material_gr',connect_el(:,2),connect_el(:,3),connect_el(:,4),connect_el(:,5)];
